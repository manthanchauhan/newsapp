...
export class NewsItem extends Component {
  getPublishedAgoString = ()=>{
    let publishedAtString = this.props.publishedAt;
    ...
    let yearDiff = Math.floor((currDate - publishedAtDate)/(1000*60*60*24*365));

    if (yearDiff > 0){
      return yearDiff + ` year${yearDiff > 1 ? "s" : ""}`;
    }

    let monthDiff = Math.floor((currDate - publishedAtDate)/(1000*60*60*24*30));

    if (monthDiff > 0){
      return monthDiff + ` month${monthDiff > 1 ? "s" : ""}`;
    }

    let dayDiff = Math.floor((currDate - publishedAtDate)/(1000*60*60*24));
  ...
  getAuthorName = ()=>{
    ...
    if (this.props.source !== null){
      let sourceName = this.props.source.name;
        ...
        <div className="card" style={{minHeight: '350px'}}>
          ...
          <div className="card-body">
            ...
            <p className="card-text">
              {this.getPublishedAgoString() && <small className="text-body-secondary">{`Published ${this.getPublishedAgoString()} ago`}</small>}
              <br/>
              <small className="text-body-secondary">{`By ${this.getAuthorName()}`}</small>
            </p>
